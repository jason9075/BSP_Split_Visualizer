...
class Visualizer:
    ...
    def draw_bsp_tree(self, node, positions, indices, depth=0):
        fig, ax = self._create_figure("BSP Tree Structure")
        self._draw_bsp_tree_node(ax, node, positions, indices, depth)
    ...
    def _draw_bsp_tree_node(self, ax, node, positions, indices, depth=0):
        node_id = id(node)
        ...
        if isinstance(node, BSPLeaf):
            ...
            for child in [node.front, node.back]:
                ...
                self._draw_bsp_tree_node(ax, child, positions, indices, depth + 1)
